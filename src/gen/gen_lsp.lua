  ...
  do -- capabilities
    ...
    for _, item in ipairs(all) do
      ...
      if
        ...
        and not found[base_provider]
        and item.clientCapability
        and item.clientCapability ~= item.serverCapability
      then
        output[#output + 1] = ("  ['%s'] = { %s },"):format(
          base_provider or item.method,
          "'" .. item.clientCapability:gsub('%.', "', '") .. "'"
        )
        found[base_provider or item.method] = true
      end
    end
